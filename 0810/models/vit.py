...
class Attention(nn.Module):
    ...
    def forward(self, x, gt_mask=None):
        ...
        attn = attn / 0.5 # low temp 
        attn = self.attend(attn)
        #print(attn.shape, gt_mask.shape) torch.Size([16, 6, 513, 513]) torch.Size([16, 1, 16, 32])
        if gt_mask!=None:
            attn_ = attn.clone()
            for h in range(attn_.shape[1]):
                attn_[:,h,0,1:] = gt_mask.reshape(B,N-1)
            x = (attn_ @ v).transpose(1, 2).reshape(B, N, C)
        else:
